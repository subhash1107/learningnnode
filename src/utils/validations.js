...
const validateSignUpData = async (req, res, next) => {
  ...
  try {
    if (!eMail || !firstName || !password) {
      ...
      throw new Error(
        "your password should contain atleast one Upper case, one lowercase, one symbol and one numeric character."
      );
...
const validateEditProfileData = async (req, res, next) => {
  try {
    ...
    const { firstName, skills, gender, age, about, photoUrl, lastName } =
      req.body;
...
export { validateSignUpData, validateEditProfileData, validatePasswordData };
